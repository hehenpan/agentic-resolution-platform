...
const LOCAL_STORAGE_ROLE_KEY = 'auth_user_type';

function getStoredUser(): { userEmail: string | null; tenantId: number | null; userType: UserRole | null } {
  if (typeof window === 'undefined') return { userEmail: null, tenantId: null, userType: null };
  ...
  const userType = localStorage.getItem(LOCAL_STORAGE_ROLE_KEY) as UserRole | null;
  return { userEmail: email, tenantId, userType };
}

function setStoredUser(email: string, tenantId: number, userType: UserRole): void {
  if (typeof window === 'undefined') return;
  localStorage.setItem(LOCAL_STORAGE_EMAIL_KEY, email);
  localStorage.setItem(LOCAL_STORAGE_TENANT_KEY, String(tenantId));
  localStorage.setItem(LOCAL_STORAGE_ROLE_KEY, userType);
}
...
export const useAuthStore = create<AuthState>((set) => ({
  ...
  userType: initialAuth ? (initialUser.userType ?? 'user') : null,
  isLoading: false,
  error: null,
  ...
  checkAuth: () => {
    ...
    set({
      ...
      userType: hasSession ? (user.userType ?? 'user') : null,
    });
    return hasSession;
  },
  ...
  login: async (credentials: LoginRequest): Promise<boolean> => {
    ...
    try {
      ...
      if (res.code === 0) {
        ...
        const userType = res.data?.user_type ?? 'user';
        setCookie(SESSION_COOKIE_NAME, `session_${Date.now()}`);
        setStoredUser(credentials.email, tenantId, userType);
        set({
      ...
      if (import.meta.env.VITE_MOCK === 'true' || process.env.NODE_ENV === 'test') {
        const mockRole = credentials.email.includes('admin') ? 'tenant_admin' : 'user';
        setCookie(SESSION_COOKIE_NAME, `mock_session_${Date.now()}`);
        setStoredUser(credentials.email, DEFAULT_MOCK_TENANT_ID, mockRole);
        set({
          isAuthenticated: true,
          userEmail: credentials.email,
